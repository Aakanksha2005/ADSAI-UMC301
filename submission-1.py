...
oof_preds_svc = np.zeros(len(X)) # New: SVC
...
test_preds_svc = np.zeros(len(X_test)) # New: SVC
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y), 1):
    ...
    # Step 1: Impute for RF, ET, AdaBoost, and SVC
    ...
    model_svc.fit(X_tr_scaled, y_tr)
    oof_preds_svc[val_idx] = model_svc.predict_proba(X_val_scaled)[:, 1]
    test_preds_svc += model_svc.predict_proba(X_test_scaled)[:, 1] / kf.n_splits
...
# Combine predictions from all 7 models
all_oof_preds = [
    oof_preds_xgb,
    oof_preds_lgb,
    oof_preds_cat,
...
auc_svc = roc_auc_score(y, oof_preds_svc)
